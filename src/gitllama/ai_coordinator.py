...
""",
    ...
    def generate_commit_message(self, operations: List[Dict[str, str]]) -> str:
        ...
        # Get project context from exploration context manager
        exploration_context = context_manager.get_context_content("repository_exploration")
        project_context = {}
        if exploration_context:
            try:
                exploration_data = json.loads(exploration_context)
                project_context = exploration_data.get('analysis', {})
            except json.JSONDecodeError:
                pass
...
Respond with ONLY the commit message, no explanation."""
        ...
        # Use context manager for this AI call
        context_manager.use_context("commit_message_generation", f"Generating commit message for {len(operations)} operations")
        
        response = ""
        for chunk in self.client.chat_stream(self.model, messages, context_name="commit_message_generation"):
            response += chunk
        ...
        # Store commit message in context manager
        context_content = json.dumps({
            "type": "commit_message",
            "message": commit_message
        }, indent=2)
        context_manager.get_or_create_context("commit_message_result", context_content)
